...
- `mert_perceptual_loss(pred, target, ...)` → cosine distance on layer-normalized embeddings
...
- `layers` — single layer (`int`) or list of layers to average (default `[6, 7, 8, 9]`)
- `normalize` — layer-normalize features (default `True`)
